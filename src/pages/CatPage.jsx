...
import { categories1 } from "../assets/data";
...
import { useGetAllPet } from "~/hooks/query/useCat";
...
const CatPage = () => {
  ...
  const data = useGetAllPet({ type: "cat" });
  ...
  //   setIsLoading(true);
  //   try {
  //     const { response, err } = await dogApi.getAll({ type: "cat" });
  //     if (response) {
  //       setData(response);
  //     }
  //     if (err) {
  ...
  return (
    <Helmet title="Mèo cảnh">
      <SectionBanner title={"Mèo cảnh"}/>
